...
/**
 * Class for creation car on the 2nd lane. date: April, 2012
 * 
 * @version 1.0
 * @see Car
 * @see Observer
 ...
 * 
 */
public class CarNumberTwo extends Applet implements Car, Observer {

	private boolean arrivedDistance = true;
	private boolean move;
	private static final long serialVersionUID = 1L;
	private int coordX = 360;
	 ...
	 * @param currentLane
	 */
	public CarNumberTwo(Lane currentLane) {
	...
	public void draw(Graphics g) {
		...
		if (move) {
			...
			if ((Runner.getListOfCars()[2].size() >= 2)
					& (Runner.getListOfCars()[2].indexOf(this) != 0)) {
				if (((Runner.getListOfCars()[2].get(Runner.getListOfCars()[2]
						.indexOf(this) - 1)).getCoordY() + 40) > coordY) {
					Runner.getListOfCars()[2].get(
							Runner.getListOfCars()[2].indexOf(this))
			...
			if (arrivedDistance) {
				...
				if (coordY == -21) {
					...
					Runner.getListOfCars()[2].remove(Runner.getListOfCars()[2]
							.indexOf(this));
				}
			}
		}

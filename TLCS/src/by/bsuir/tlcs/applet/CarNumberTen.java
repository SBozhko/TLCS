...
/**
 * Class for creation car on the 10th lane. date: April, 2012
 * 
 * @version 1.0
 * @see Car
 * @see Observer
 ...
 * 
 */
public class CarNumberTen extends Applet implements Car, Observer {

	private boolean arrivedDistance = true;
	private boolean move;
	private static final long serialVersionUID = 1L;
	private int coordX = 0;
	private int coordY = 268;
	 ...
	 * @param currentLane
	 */
	public CarNumberTen(Lane currentLane) {
	...
	public void draw(Graphics g) {
		...
		if (move) {
			...
			if ((Runner.getListOfCars()[10].size() >= 2)
					& (Runner.getListOfCars()[10].indexOf(this) != 0)) {
				if (((Runner.getListOfCars()[10].get(Runner.getListOfCars()[10]
						.indexOf(this) - 1)).getCoordX() - 40) < coordX) {
					if (((Runner.getListOfCars()[10]
							.get(Runner.getListOfCars()[10].indexOf(this) - 1))
							.getCoordY()) < 268) {
						Runner.getListOfCars()[10].get(
								Runner.getListOfCars()[10].indexOf(this))
			...
			if (arrivedDistance) {
				...
				if (coordX <= 270) {
					...
					g2.clipRect(coordX, coordY, 1, 20);
				} else {
					g2.drawImage(secondCar, coordX, coordY, this);
					g2.clipRect(coordX, coordY, 1, 20);
					paint(g2);
					g2.setClip(coordX, coordY + 20, 20, 1);
				}
				paint(g2);
				...
				if (coordY == -21) {
					...
					Runner.getListOfCars()[10]
							.remove(Runner.getListOfCars()[10].indexOf(this));
				}
			}
		}
	}

...
/**
 * Class for creation car on the 14th lane. Lane for pedestrians. Connecting to
 ...
 * 
 */
public class CarNumberFourteen extends Applet implements Car, Observer {

	private boolean arrivedDistance = true;
	private boolean move;
	private static final long serialVersionUID = 1L;
	private int coordX = 418;
	 ...
	 * @param currentLane
	 */
	public CarNumberFourteen(Lane currentLane) {
	...
	public void draw(Graphics g) {
		...
		if (move) {
			...
			if ((Runner.getListOfCars()[14].size() >= 2)
					& (Runner.getListOfCars()[14].indexOf(this) != 0)) {
				if (((Runner.getListOfCars()[14].get(Runner.getListOfCars()[14]
						.indexOf(this) - 1)).getCoordY() - 40) < coordY) {
					Runner.getListOfCars()[14].get(
							Runner.getListOfCars()[14].indexOf(this))
			...
			if (arrivedDistance) {
				...
				if (coordY == 490) {
					...
					Runner.getListOfCars()[14]
							.remove(Runner.getListOfCars()[14].indexOf(this));
				}
			}
		}
	}

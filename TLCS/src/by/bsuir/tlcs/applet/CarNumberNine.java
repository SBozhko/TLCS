...
/**
 * Class for creation car on the 9th lane. date: April, 2012
 * 
 * @version 1.0
 * @see Car
 * @see Observer
 ...
 * 
 */
public class CarNumberNine extends Applet implements Car, Observer {

	private boolean arrivedDistance = true;
	private boolean move;
	private static final long serialVersionUID = 1L;
	private int coordX = 215;
	 ...
	 * @param currentLane
	 */
	public CarNumberNine(Lane currentLane) {
	...
	public void draw(Graphics g) {
		...
		if (move) {
			...
			if ((Runner.getListOfCars()[9].size() >= 2)
					& (Runner.getListOfCars()[9].indexOf(this) != 0)) {
				if (((Runner.getListOfCars()[9].get(Runner.getListOfCars()[9]
						.indexOf(this) - 1)).getCoordY() - 40) < coordY) {
					if (((Runner.getListOfCars()[9]
							.get(Runner.getListOfCars()[9].indexOf(this) - 1))
							.getCoordX()) < 215) {
						Runner.getListOfCars()[9].get(
								Runner.getListOfCars()[9].indexOf(this))
			...
			if (arrivedDistance) {
				...
				if (coordX == -21) {
					...
					Runner.getListOfCars()[9].remove(Runner.getListOfCars()[9]
							.indexOf(this));
				}
			}
		}

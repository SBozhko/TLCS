...
/**
 * Class for creation car on the 4th lane. date: April, 2012
 * 
 * @version 1.0
 * @see Car
 * @see Observer
 ...
 * 
 */
public class CarNumberFour extends Applet implements Car, Observer {

	private boolean arrivedDistance = true;
	private boolean move;
	private static final long serialVersionUID = 1L;
	private int coordX = 670;
	private int coordY = 231;
	 ...
	 * @param currentLane
	 */
	public CarNumberFour(Lane currentLane) {
	...
	public void draw(Graphics g) {
		...
		if (move) {
			...
			if ((Runner.getListOfCars()[4].size() >= 2)
					& (Runner.getListOfCars()[4].indexOf(this) != 0)) {
				if (((Runner.getListOfCars()[4].get(Runner.getListOfCars()[4]
						.indexOf(this) - 1)).getCoordX() + 40) > coordX) {
					if (((Runner.getListOfCars()[4]
							.get(Runner.getListOfCars()[4].indexOf(this) - 1))
							.getCoordY()) > 231) {
						Runner.getListOfCars()[4].get(
								Runner.getListOfCars()[4].indexOf(this))
			...
			if (arrivedDistance) {
				...
				if (coordX >= 340) {
					...
					g2.clipRect(coordX + 20, coordY - 1, 1, 20);
					paint(g2);
					g2.setClip(coordX, coordY - 1, 20, 1);
				}
				paint(g2);
				...
				if (coordY == 480) {
					...
					Runner.getListOfCars()[4].remove(Runner.getListOfCars()[4]
							.indexOf(this));
				}
			}
		}
